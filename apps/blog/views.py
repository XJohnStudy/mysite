...
def get_blog_list_common_data(request, blogs_all_list):
  paginator = Paginator(blogs_all_list, settings.EACH_PAGE_NUM) # 每10篇为一页
  page_num = request.GET.get('page', 1) # 获取页码参数（GET请求）
  page_of_blogs = paginator.get_page(page_num) # 自动识别（默认为1）
  ...
  blog_types_list = BlogType.objects.annotate(blog_count=Count('blog')) # 对象Blog的小写
...
def blog_detail(request, blog_pk):
  ...
  # context['user'] = request.user # 下面使用render可以不加这一句
  response = render(request, 'blog_detail.html', context) # 响应
  from datetime import datetime
  response.set_cookie(read_cookie_key, 'true', max_age=60, expires=datetime) # 阅读标记
  return response
